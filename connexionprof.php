<?php
   ...
   if(!empty($_POST['email']) AND !empty($_POST['password'])   ){
    ...
    $recupUser = $bdd->prepare('SELECT * FROM prof WHERE email = ? AND `password` = ?' );
    ...
    if($recupUser->rowCount() > 0){
      ...
      $_SESSION['id_prof'] = $recupUser->fetch()['id_prof'];

      $plus = $bdd->prepare('SELECT * FROM prof WHERE id_prof = ?');
      $plus->execute(array($_SESSION['id_prof']));
      $row = $plus->fetch();
...
<body>
  ...
  <form action="" method="post">
    ...
    <p class="passe oublié"><p>J'ai oublié mon mot de passe <span><a href=""> cliquez ici pour réinitialiser</a></span></p>
		    ...
		    <p><a href="inscriptionprof.php"  class="inscriptionprof">Pour m'inscrire c'est ici</a></p>
